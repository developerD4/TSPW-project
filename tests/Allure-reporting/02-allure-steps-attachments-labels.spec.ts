...
test('OrangeHRM login with readable Allure steps', async ({ page }) => {
  ...
  await allure.step('Enter login details', async (step) => {
    // Parameter = Shows test data in Allure report
    await step.parameter('username', 'Admin'); //Step Parameters
...
test('add custom Allure metadata', async ({ page }) => {
  ...
  await allure.parameter('environment', 'demo'); //Test-level Parameters
