...
test('Find a menu item using an element inside it', async ({ page }) => {
    ...
    const menuItem = page
        ...
        .filter({ hasText: 'PIM' });
    //hasText → Element must contain the specified text
    // Has → Element must contain the specified locator
    // hasNotText → Element must NOT contain the specified text
    // hasNot → Element must NOT contain the specified locator
...
test('Find an element inside the main menu', async ({ page }) => {
    ...
    const pimMenu = menu.getByText('PIM', { exact: true });
