...
//Explanation -`waitForEvent()` tells Playwright to wait until a specific event happens
//  and then gives us the related object/data.**
...
// // newTab.waitForLoadState() → Waits until the newly opened tab finishes loading before we interact with or verify its elements.
...
test('Handle multiple tabs', async ({ page, context }) => {
    ...
    const firstTab = page;
    // Check the first tab
    await expect(firstTab.getByRole('heading', { name: 'Form Elements' }))
        .toHaveText('Form Elements');
    // Check the new tab
    await expect(secondTab.getByRole('heading'))
